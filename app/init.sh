...
# --- Fix static symlinks after volume mount ---
if [ -d "$DALORADIUS_PATH/app/common/static" ]; then
    mkdir -p "$DALORADIUS_PATH/app/operators"
    rm -rf "$DALORADIUS_PATH/app/operators/static"
    ln -s "$DALORADIUS_PATH/app/common/static" \
          "$DALORADIUS_PATH/app/operators/static"

    mkdir -p "$DALORADIUS_PATH/app/users"
    rm -rf "$DALORADIUS_PATH/app/users/static"
    ln -s "$DALORADIUS_PATH/app/common/static" \
          "$DALORADIUS_PATH/app/users/static"
fi
