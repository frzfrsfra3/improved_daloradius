...
function update_user_billing_info($dbSocket, $username, $params) {
    $allowedFields = array(
                            ...
                            "emailinvoice", "batch_id", "creationdate", "creationby", "updatedate", "updateby","planName"
                          );
...
function add_user_billing_info($dbSocket, $username, $params) {
    $allowedFields = array(
                           "planName", "id", "username", "planName", "hotspot_id", "hotspotlocation", "contactperson", "company",
                            "email", "phone", "address", "city", "state", "country", "zip", "paymentmethod", "cash",
                            "creditcardname", "creditcardnumber", "creditcardverification", "creditcardtype",
                            "creditcardexp", "notes", "changeuserbillinfo", "lead", "coupon", "ordertaker", "billstatus",
                            "lastbill", "nextbill", "nextinvoicedue", "billdue", "postalinvoice", "faxinvoice",
...
/**
 * Apply plan quota to user (Max-Total-Octets)
 *
 * @param object $dbSocket
 * @param string $username
 * @param string $planName
 * @return bool
 */
function apply_plan_quota_to_user($dbSocket, $username, $planName) {
    global $configValues, $logDebugSQL;

    $username = trim($username);
    $planName = trim($planName);

    if (empty($username) || empty($planName)) {
        return false;
    }

    // 1) get quota from billing plan
    $sql = sprintf(
        "SELECT planQuotaMB FROM %s WHERE planName='%s'",
    ...
    // 3) insert or update radcheck
    $sql = sprintf(
        "INSERT INTO %s (id, username, attribute, op, value)
         VALUES (0, '%s', 'Max-Total-Octets', ':=', '%s')
